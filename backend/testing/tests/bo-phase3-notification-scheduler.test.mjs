...
describe('Phase 3 notification scheduler', () => {
  test('envia recordatorios en ventana configurable y deduplica ejecuciones repetidas', async () => {
    ...
    expect(Number(firstRun.processed_by_user_id)).toBe(Number(fixture.user_id));
    expect(Number(firstRun.audit_id)).toBeGreaterThan(0);
  ...
  test('rechaza ejecucion batch sin processed_by_user_id', async () => {
    await expect(
      sendReturnReminderBatch({
        window_hours: 2,
        dedup_hours: 24,
