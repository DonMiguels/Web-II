...
router.post('/register', async (req, res) => {
  try {
    ...
    res.status(STATUS_CODES.BAD_REQUEST).json({
      message: config.getMessage(config.LANGUAGE, 'registration_error'),
      error,
    });
  }
});
...
router.post('/login', async (req, res) => {
  try {
    ...
    res
      ...
      .json({ message: config.getMessage(config.LANGUAGE, 'server_error'), error });
  }
});
...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    const userData = await userService.resetPasswordByUsername({ username, password });
    if (!userData) {
      return res
...
router.post('/logout', async (req, res) => {
  ...
  return res
    ...
    .json({ message: result?.message || config.getMessage(config.LANGUAGE, 'logout_success') });
});
